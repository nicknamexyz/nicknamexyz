...
Theory: Infix notation is the conventional way of writing expressions where 
operators are placed between operands (e.g., A + B). Postfix notation, or Reverse 
Polish Notation (RPN), eliminates the need for parentheses by placing operators 
after their operands (e.g., AB+). This allows for straightforward evaluation using a 
stack, where operands are pushed onto the stack and operators pop operands for 
computation. 
 
Algorithm: 
1. Define a stack structure to hold characters and evaluate expressions. 
2. Implement a function to convert infix expressions to postfix: 
   - Use the precedence of operators to determine the order. 
   - Utilize the stack to temporarily hold operators and output the postfix 
expression. 
3. Implement a function to evaluate the postfix expression: 
   - Use a stack to store operands and compute results based on operators. 
4. In the main function, prompt the user for an infix expression, perform the 
conversion, and evaluate the postfix expression.
